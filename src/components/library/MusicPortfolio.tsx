    ...
    if (sortedTracks.length === 0) {
        return (
            <div className="flex items-center justify-center h-full" style={{ background: '#090907' }}>
                <p className="text-white/60 text-[11px] font-mono uppercase tracking-[0.2em]">No tracks to display</p>
            </div>
        );
    }
        ...
        >
            ...
            {onExit && (
                <button
                    ...
                    className="absolute top-4 right-4 md:top-6 md:right-6 z-20 flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-black/55 backdrop-blur border border-white/[0.08] text-[10px] font-mono uppercase tracking-[0.2em] text-white/80 hover:text-white hover:border-white/[0.18] transition-colors"
                >
                            ...
                            >
                                ...
                                <div className="w-6 shrink-0 flex items-center justify-center">
                                    {isCurrent && isPlaying ? (
                                        ...
                                        <span
                                            className={`text-[11px] font-mono tabular-nums ${isHovered ? 'text-white' : 'text-white/60'
                                                }`}
                                        >
                                ...
                                {/* Type */}
                                <span className="hidden md:block text-[11px] font-mono uppercase tracking-[0.2em] text-white/60 group-hover:text-white transition-colors duration-300 w-20 shrink-0">
                                    {track.type}
                                </span>

                                {/* BPM · Key (with minor suffix) */}
                                <span className="hidden sm:block text-[11px] font-mono uppercase tracking-[0.2em] text-white/60 group-hover:text-white transition-colors duration-300 w-24 shrink-0 tabular-nums">
                                    {(track.bpm ?? '-')}{track.key ? ` · ${track.key}${track.scale === 'minor' ? 'm' : ''}` : ''}
                                </span>

                                {/* Year */}
                                <span className="text-[11px] font-mono uppercase tracking-[0.2em] text-white/60 group-hover:text-white transition-colors duration-300 w-12 shrink-0 text-right">
                                    {track.year}
                                </span>
                                ...
                                {isEmbedded && (
                                    <div className="hidden md:flex items-center gap-2.5 shrink-0 pointer-events-none">
                                        ...
                                        {track.durationSeconds != null && (
                                            <span className="text-[11px] font-mono text-white/80 tabular-nums w-10 text-right">
                                        ...
                                        {track.freeDownload ? (
                                            ...
                                            <div className="flex items-center gap-1.5">
                                                ...
                                                {track.priceLease == null && track.priceExclusive == null && (
                                                    <span className="text-[11px] font-mono text-white/40">No price</span>
                                                )}
                                            </div>
                                        )}
                                        {/* Wishlist heart */}
