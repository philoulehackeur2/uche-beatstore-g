...
function StorePage() {
  ...
  // Cart + global player.
  const { items, addItem, removeItem, clearCart, isOpen, setIsOpen, cartTotal } = useCart();
  ...
  return (
    <div className="min-h-screen bg-[#0a0907] text-[#E8DCC8]">
      ...
      {isOpen && (
        <CartDrawer onClose={() => setIsOpen(false)} items={items} removeItem={removeItem} total={cartTotal()} />
      )}
    </div>
  );
}
...
function CartDrawer({
  onClose, items, removeItem, total,
}: {
  onClose: () => void;
  items: Array<{ id: string; track: Track; license: { name: string; price_usd: number; is_exclusive: boolean } }>;
    ...
    <>
      ...
      <aside className="fixed top-0 right-0 bottom-0 z-[90] w-full sm:w-[420px] bg-gradient-to-b from-[#101012]/95 via-[#0a0907]/95 to-[#0a0907]/95 backdrop-blur-2xl border-l border-white/[0.06] shadow-[-12px_0_40px_rgba(0,0,0,0.5)] flex flex-col animate-in slide-in-from-right duration-300">
        ...
        <div className="flex-1 overflow-y-auto px-3 py-3">
          {items.length === 0 ? (
            ...
            <ul className="space-y-2">
              {items.map((i) => (
                <li key={i.id} className="flex items-center gap-3 p-3 rounded-lg bg-white/[0.02] border border-white/[0.05]">
                  <div className="w-10 h-10 rounded bg-[#0a0907] border border-[#1f1a13] overflow-hidden shrink-0">
                    {i.track.cover_url ? <img loading="lazy" src={i.track.cover_url} alt="" className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-[#3a3328]"><Music size={14} /></div>}
                  </div>
        ...
        <div className="border-t border-white/[0.04] px-5 py-4 space-y-3 bg-[#0a0907]/40">
          ...
          >
            {checkoutLoading ? <Loader2 size={13} className="animate-spin" /> : <ShoppingCart size={13} />}
            Checkout
          </button>
