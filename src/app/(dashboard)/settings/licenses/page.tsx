...
import {
  ArrowLeft, Plus, Trash2, GripVertical, Loader2, Check,
  DollarSign, ChevronDown, ChevronUp, Music, FileText, Globe,
...
export default function LicensesSettingsPage() {
  ...
  // Draft helpers — local edits don't hit the server until Save is clicked
  function getDraft(l: License): License {
    return { ...l, ...drafts[l.id] } as License;
  }

  function patch(id: string, field: keyof License, value: any) {
  ...
  // Drag-and-drop sort order update (simple swap on arrow buttons)
  async function moveUp(idx: number) {
    if (idx === 0) return;
    const reordered = [...licenses];
    [reordered[idx - 1], reordered[idx]] = [reordered[idx], reordered[idx - 1]];
    setLicenses(reordered.map((l, i) => ({ ...l, sort_order: i })));
    // Persist both swap partners
    await Promise.all([
  ...
  return (
    <DashboardLayout>
      <div className="max-w-[780px] mx-auto px-4 sm:px-6 md:px-10 pt-6 md:pt-10 pb-32">
        ...
        {/* Explainer */}
        <div className="rounded-xl border border-[#1f1a13] bg-[#14110d] p-4 mb-6 flex gap-3">
          <AlertCircle size={14} className="text-[#a08a6a] shrink-0 mt-0.5" />
          <div className="text-[11px] text-[#6a5d4a] leading-relaxed space-y-1">
            <p>
              ...
              Tiers are ordered here and reflected live on your store.
            </p>
            <p>
              Track-level price overrides in the library take precedence over global tier prices.
              Existing "Lease / Exclusive" defaults in Creator Profile still apply to tracks with no tiers linked.
            </p>
          </div>
        </div>
        ...
        {/* License list */}
        {loading ? (
          <div className="flex items-center justify-center py-24 text-[#5a5142]">
                  ...
                  {isExpanded && (
                    <div className="px-4 pb-5 space-y-5 border-t border-[#1f1a13] pt-4">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div>
                          <label className="field-label">License Name</label>
                          <input
                            type="text"
                            maxLength={80}
                            value={draft.name}
                      ...
                      {/* File types */}
                      <div>
                        <label className="field-label">Files Included</label>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div>
                          <label className="field-label">Streaming Limit</label>
                          <select
                        ...
                        <div>
                          <label className="field-label">Distribution Limit</label>
                          <select
                      ...
                      {/* Rights toggles */}
                      <div>
                        <label className="field-label mb-2">Rights</label>
                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                          {[
                            ...
                            const on = !!(draft as any)[field];
                            return (
                              <button
                                key={field}
                                onClick={() => patch(l.id, field as keyof License, !on)}
                              ...
                              >
                                ...
                                {on && <Check size={9} className="ml-auto" />}
                              </button>
                            );
                          })}
                        </div>
      ...
      {/* Scoped utility classes for form fields */}
      <style jsx global>{`
        .field-label {
          display: block;
          font-size: 9px;
        ...
        .field-input::placeholder { color: #3a3328; }
        .field-input:focus { border-color: #8A7A5C; }
      `}</style>
    </DashboardLayout>
  );
}
